...
public class MaryHttpServer extends Thread
{
    ...
    public void run()
    {
        ...
        MivoqServerInfoHandler MivoqinfoRH = new MivoqServerInfoHandler();
	// Check version information
        registry.register("/info/version", MivoqinfoRH);
	// Check voices availability
        registry.register("/info/voices/all", MivoqinfoRH);
        registry.register("/info/voices/locale/*", MivoqinfoRH);
        registry.register("/info/locales/all", MivoqinfoRH);
	// Check voice capabilities
        registry.register("/info/voice/*", MivoqinfoRH);
	// Request synthesis
        registry.register("/say", new MivoqSynthesisRequestHandler());
        ...
        try {
            final ListeningIOReactor ioReactor = new DefaultListeningIOReactor(numParallelThreads, params);
	    Runtime rt = Runtime.getRuntime();
	    rt.addShutdownHook(new Thread() { public void run() { try { ioReactor.shutdown(); } catch (IOException e) { } }; });
