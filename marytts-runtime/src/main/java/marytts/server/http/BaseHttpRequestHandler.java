...
public abstract class BaseHttpRequestHandler extends SimpleNHttpRequestHandler implements HttpRequestHandler  
{
    ...
    throws HttpException, IOException
    {
        try {
	      ...
	      MaryHttpServerUtils.toKeyValuePairs(query, true, queryItems);
            }
            
            //Try and get parameters from different HTTP POST requests
            if (method.equals("POST")
                    && request instanceof HttpEntityEnclosingRequest) {
                try {
                    String postQuery = EntityUtils.toString(((HttpEntityEnclosingRequest) request).getEntity());
		    MaryHttpServerUtils.toKeyValuePairs(postQuery, true, queryItems);
                } catch (Exception e) {
