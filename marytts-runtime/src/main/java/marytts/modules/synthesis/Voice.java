...
public class Voice
{
  ...
  public boolean supportsLocale(Locale l){
    Locale lv = this.getLocale();
  ...
  public int localeDistance(Locale l){
    if(l==null)
      return -1;
    Locale lv = this.getLocale();
    int dist = 0;
  ...
  private static class VoiceComparator implements Comparator<Voice> {
    Locale locale;
    Gender gender;
    HashMap<String,Integer> names = null;
    int max_names = 0;
    int age;
    VoiceComparator(Locale locale, Gender gender, String[] names, int age){
      this.locale = locale;
      this.gender = gender;
      if (names != null) {
	this.names = new HashMap<String,Integer>();
	for(String v: names){
	  if(!this.names.containsKey(v)) {
	    this.names.put(v, this.max_names);
	    this.max_names++;
	  }
	}
	this.age = age;
    ...
    public int compare(Voice v2, Voice v1) {
      boolean v1_win, v2_win;
      int dist1, dist2;
      // Compare according to Locale distance
      if(this.locale != null) {
	v1_win = v1.supportsLocale(this.locale);
	v2_win = v2.supportsLocale(this.locale);
	if(v1_win && !v2_win)
	  return 1;
	else if(!v1_win && v2_win)
	  return -1;
	else if(!v1_win && !v2_win) {
	  dist1 = v1.localeDistance(this.locale);
	  dist2 = v2.localeDistance(this.locale);
	  if(dist1 < dist2){
	    return 1;
	  }
	  else if(dist1 > dist2){
      ...
      // Compare according to Gender distance
      if(this.gender !=null) {
	v1_win = v1.gender.equals(this.gender);
	v2_win = v2.gender.equals(this.gender);
	if(v1_win && !v2_win)
	  return 1;
	else if(!v1_win && v2_win)
	  return -1;
	else if(!v1_win && !v2_win) {
	  if(!this.gender.equals(NEUTRAL)) {
	    v1_win = v1.gender.equals(NEUTRAL);
	    v2_win = v2.gender.equals(NEUTRAL);
	    if(v1_win && !v2_win)
	      return 1;
	    else if(!v1_win && v2_win)
	      return -1;
      ...
      // Compare according to Name list
      if(this.names !=null) {
	v1_win = this.names.containsKey(v1.voiceName);
	v2_win = this.names.containsKey(v2.voiceName);
      if(v1_win && !v2_win)
	return 1;
      else if(!v1_win && v2_win)
	return -1;
      else if(v1_win && v2_win) {
	  dist1 = this.names.get(v1.voiceName);
	  dist2 = this.names.get(v2.voiceName);
	  if(dist1 < dist2){
	    return 1;
	  }
	  else if(dist1 > dist2){
      ...
      // Age not supported yet
      // Return negative number if v1 should be listed before v2
      int desireDelta = v1.wantToBeDefault - v2.wantToBeDefault;
      if (desireDelta != 0) return desireDelta;
      // If no parameter makes any difference, sort alphabetically
  ...
  /**
	 ...
	 * @return
	 */
  public static List<Voice> getVoiceWithSSMLAlgorythm(Locale locale, Gender gender, String[] names, int age)
    {
        ArrayList<Voice> list = new ArrayList<Voice>();
	for (Iterator<Voice> it = allVoices.iterator(); it.hasNext();) {
	  Voice v = (Voice) it.next();
	  if(v.localeDistance(locale)<100){
	    list.add(v);
	  }
	}
	Collections.sort(list, new VoiceComparator(locale, gender, names, age));
        return list;
    }
