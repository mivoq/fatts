...
public abstract class ProducingDoubleDataSource extends BufferedDoubleDataSource implements Runnable {
    ...
    /**
     * Close and interrupt the producing data source
     */
    public void close() {
    	// In order to generate an InterruptedException in the queue.put thread, the solution is to interrupt the thread
    	dataProducingThread.interrupt();
    	// Clear the queue and putEndOfStream to close the eventually queue.take
    	queue.clear();
    	putEndOfStream();
    }
