...
public class FST
{
    ...
    throws IOException, UnsupportedEncodingException
    {
        ...
        int OVERALL_BITS = in.readInt();
        int ARCOFFSET_BITS = in.readInt();
        ...
        int maskA0 = 0xFFFFFFFF >>> (OVERALL_BITS-ARCOFFSET_BITS); //1048575
        int maskLID = 0xFFFFFFFF >>> (OVERALL_BITS-LABELID_BITS); //2047

        for(i=0; i<nArcs; i++) {
            int thisArc = in.readInt();
            
            targets[i]= thisArc&maskA0;
            labels[i]=(short)((thisArc>>ARCOFFSET_BITS) & maskLID);
            isLast[i]=((byte)(thisArc >> (OVERALL_BITS-1)))!=0;
