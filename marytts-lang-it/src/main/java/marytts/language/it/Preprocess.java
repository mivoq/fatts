...
public class Preprocess extends InternalModule
{
    ...
    throws Exception
    {
		...
		//logger.info("ImproveTokenization...");
        //improveTokenization(doc);
 	...
 	/* 
	 * This method try to set a paragraph for every sentence (Van be used for low latency synthesis)
	 * Actually not used because a split on chunks has already done before :(
	 */
    private void improveTokenization(Document doc)
    {
        NodeList maryxml = doc.getElementsByTagName(MaryXML.MARYXML);
        
        for(int i=0; i<maryxml.getLength(); i++) {
            Node child = maryxml.item(i).getFirstChild();
            while(child!=null)
            {
            if(MaryXML.PARAGRAPH.equals(child.getNodeName())){
            	//espandere paragrafo
            	NodeList nl = child.getChildNodes();
            	Node newChild;
                for(int j=0; j<nl.getLength(); j++) {
                	newChild = child.cloneNode(false);
                	newChild.appendChild(nl.item(j).cloneNode(true));
                	maryxml.item(i).insertBefore(newChild, child);
                }
            	// skip to next node
            	newChild = child.getNextSibling();
            	maryxml.item(i).removeChild(child);
            	child = newChild;
            } else {
            	// skip to next node
