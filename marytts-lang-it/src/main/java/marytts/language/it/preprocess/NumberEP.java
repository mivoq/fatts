...
public class NumberEP extends ExpansionPattern
{
    ...
    //protected static final String sCodes = "(?:[0-9.,_/\\\\-]*[0-9][.,_/\\\\-]?)";
    protected static final String sCodes = "(?:(?:[0-9_-]|\\W)*[0-9](?:[_-]|\\W)?)";
    ...
    protected final Pattern reCodes = Pattern.compile(sCodes);
    ...
    protected boolean matchCodes(String s)
    {
        return reCodes.matcher(s).matches();
    }
    ...
    protected String expandFloat(String number)
    {
        ...
        int i;
		for (i = 0; i < number.length() && Character.isSpace(number.charAt(i)); i++);
		if(i < number.length() && number.charAt(i) == '-') i++;
		for (; i < number.length() && Character.isDigit(number.charAt(i)); i++);
		if( i > 0 ){
			buf.append(expandInteger(Long.parseLong(number.substring(0,i)), true));
	        buf.append(' ');
		}
    ...
    protected String expandDigits(String digits)
    {
        ...
        for (int i=0; i<digits.length(); i++) {
			...
			case '/': buf.append("barra "); break;
			case '\\': buf.append("barra "); break;
			case '_': buf.append("trattino basso "); break;
			case '-': buf.append("meno "); break;
			case '.':
